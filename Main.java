...
public class Main {
	...
	public static void header() throws IOException, InterruptedException {
		...
		System.out.println("ML-M3: ACT1 Fixed Partition - First Fit");
  ...
  public static void newSet(String dataHighlight[][], String oldData[][], String partitionTaken[], float partitions[],
    ...
    boolean validLoop = true;
		do {
			...
			for (int i = 0; i < jobQuantity; i++) {
				boolean availablePartition = false;
				int leastPartition = 0;
				int temporaryPartition = 0;
				int comparison = 0;
				if (jobAllocated[i] == 0) {
					for (int j = 0; j < partitionQuantity; j++) {
						if (jobs[i] <= partitions[j] && partitionTaken[j] == "") {

							availablePartition = true;
							comparison++;

							if (comparison > 1) {
								leastPartition = j;
								if (partitions[temporaryPartition] <= partitions[leastPartition]) {
									leastPartition = temporaryPartition;
								}
							} else {
								leastPartition = j;
								temporaryPartition = j;
							}

							for (int k = 0; k < partitionQuantity; k++) {
								if (partitions[leastPartition] > partitions[k] && jobs[i] < partitions[k] && partitionTaken[k] == "") {
									availablePartition = false;
									break;
								}
							}
						}
					...
					if (availablePartition) {
						...
						partitionTaken[leastPartition] = ("J" + String.valueOf(i + 1));
						rank++;
					}
				}
				if (rank == partitionQuantity) {
					break;
				}
			}
      ...
      validLoop = false;
			...
			for (int i = 0; i < 2; i++) {
				String firstText = allocationOrder[i];
				for (int j = 0; j < partitionQuantity; j++) {
			...
			int i = 0;
			for (int j = 1; j < partitionQuantity; j++) {
				if (allocationOrder[i] == "" && allocationOrder[j] != "") {
					allocationOrder[i] = allocationOrder[j];
					i++;
				}
			}
